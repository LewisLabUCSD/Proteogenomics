...
#     print ':::::::java -Xmx3500M -cp '+system_folder+'/MSGFPlus.20130403/MSGFPlus.jar edu.ucsd.msjava.msdbsearch.BuildSA -d '+db+'/prep/'+os.path.splitext(db_piece)[0]+'/'+db_piece+' -tda 2'
    ...
    os.system('java -Xmx3500M -cp '+system_folder+'/MSGFPlus.20130403/MSGFPlus.jar edu.ucsd.msjava.msdbsearch.BuildSA -d '+db+'/prep/'+os.path.splitext(db_piece)[0]+'/'+db_piece+' -tda 2')
    return
...
for db in database[::-1]:
    ...
    index += 1
    db_library.append([])
    os.system('mkdir '+db+'/sum')
    os.system('mkdir '+db+'/split')
    os.system('mkdir '+db+'/prep')
    os.system('mkdir '+db+'/result')
    filename = db.split('/')[-1]+'.fa'
    if len(filelist)>1:
        os.system('java -cp '+system_folder+'/../../scripts/2012.0815/CCMSWorkflowUtils.jar edu.ucsd.workflow.Merge -input '+db+' -output '+db+'/sum/'+filename)
    else:
        os.system('cp '+db+'/'+filelist[0]+' '+db+'/sum/'+filename)
    SplitBySize(max_db_size,os.listdir(db+'/sum')[0],db)
    my_list = os.listdir(db+'/split')
    for db_piece in my_list:
        db_library[index].append(db+'/prep/'+os.path.splitext(db_piece)[0]+'/'+db_piece)
        BuildSA(db_piece,db)
        print db_piece,db
...
result_list = MultiStageFDR(db_order,main_folder)
